...
def get_tower_summary_dict(summary):
    ...
    for v, method in summary:
        if len(tf.compat.v1.get_collection(v)) == 1:
            ret[v] = tf.compat.v1.get_collection(v)[0]
        elif len(tf.compat.v1.get_collection(v)) > 1:
            if method == 'mean':
                ret[v] = tf.reduce_mean(input_tensor=tf.compat.v1.get_collection(v), axis=0)
            elif method == 'sum':
                ret[v] = tf.reduce_sum(input_tensor=tf.compat.v1.get_collection(v), axis=0)
            elif method == 'concat':
                ret[v] = tf.concat(tf.compat.v1.get_collection(v), axis=0)
            else:
