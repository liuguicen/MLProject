    ...
    def make_network(self, is_train):
        ...
        if is_train:
            ...
            for i, (global_out, label) in enumerate(zip(global_outs, labels)):
                global_label = label * tf.cast(tf.greater(tf.reshape(valids, (-1, 1, 1, cfg.nr_skeleton)), 1.1), dtype=tf.float32)
                global_loss += tf.reduce_mean(input_tensor=tf.square(global_out - global_label)) / len(labels)
            ...
            refine_loss = tf.reduce_mean(input_tensor=tf.square(refine_out - label7), axis=(1,2)) * tf.cast((tf.greater(valids, 0.1)), dtype=tf.float32)
