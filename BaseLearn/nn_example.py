...
from  minist_data import MnistDataProcessor
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 255, 5)  # 二维的卷积，这两个属性应该是自带的，
        ...
        self.conv2 = nn.Conv2d(255, 432, 5)
        self.fc1 = nn.Linear(432 * 5 * 5, 331)
        self.fc2 = nn.Linear(331, 285)
        self.fc3 = nn.Linear(285, 10)
...
def training(data_processor):
    ...
    train_loader = data_processor.load_training_data()
...
def test_all(net, testloader, classes):
    correct = 0
    total = 0
    with torch.no_grad():
...
if __name__ == '__main__':
    data_processor = nn_learn_data.DataProcessor()
    net = None
    try:
        net = torch.load('.\models\model_digit.pkl')
        print('load net from disk')
    except:
        net = training(data_processor)
        # 保存
        # net = Net()
    ...
    # test_all(net, data_processor.test_data, data_processor.classes)
