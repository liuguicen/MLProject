...
# 参数解析器的使用 argparse ArgumentParser：  
parser = argparse.ArgumentParser()  
这个玩意真不咋地，除了无法定位跳转到定义，好几个坑的地方  
1、定义的参数里面的减号-可以自动替换成下划线_  我用得着你多此一举，都最后结果别人找不到谁对应谁，find操作也不好找  
2、参数不用输全就可以起作用，这多此一举吧，说不定别人根本就不想设置这个参数呢？？？   
3、默认参数，只能在参数名前面加了--的情况下才能生效，否则不生效
4、运行的python文件多了，或者跨设备，参数就没了，结果又需要重新找并设置一遍参数，所以通常不要使用这个垃圾工具，遇到使用它的开源项目，要把参数写到对应文件或者改造成不使用它的
垃圾，但是用的人多，只能学
--dest 表示
