...
def main():
    ...
    if args.demo == "image":
        ...
        while True:
            ...
            if ret_val:
                logging.error('nanodet start')
                meta, res = predictor.inference(frame)
                logging.error('nanodet finish')
...
class NanoHumanDetector():
    def __init__(self):
        ...
        logger = Logger(local_rank, use_tensorboard=False)
        self.predictor = Predictor(cfg, self.args.model, logger, device="cuda:0")
        logger.log('Press "Esc", "q" or "Q" to exit.')
        current_time = time.localtime()
        if os.path.isdir(self.args.path):
            files = get_image_list(self.args.path)
        else:
