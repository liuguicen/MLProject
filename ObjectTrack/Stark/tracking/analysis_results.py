...
def evaluateStarck():
    trackers = []
    dataset_name = 'tc128'
    """stark 创建跟踪器包装对象，这个就是单纯用于获取跟踪器的信息"""
...
def evaluateLightTrack():
    trackers = []
    dataset_name = 'PoseTrack2018'
    """stark 创建跟踪器包装对象，这个就是单纯用于获取跟踪器的信息"""
    trackers.extend(trackerlist(name='LightTrack', parameter_name='baseline', dataset_name=dataset_name,
                                run_ids=None, display_name='LightTrack'))
    dataset = get_dataset('PoseTrack2018')
    print_results(trackers, dataset, dataset_name, merge_results=True, plot_types=('success', 'prec'))
