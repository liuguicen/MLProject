...
class SamePad2d(nn.Module):
    ...
    #为了保证卷积以后水平尺寸不变,而进行padding的padding，可能是早期版本的pytorch不支持这样的操作
    """
...
class FPN(nn.Module):
    def __init__(self, C1, C2, C3, C4, C5, out_channels):
        ...
        self.C4 = C4
        self.C5 = C5
        # P5经过步长为3的max_pooling得到P6，但P6只用于在RPN中用来得到region prposal
    ...
    def forward(self, x):
        # 注意直接通过FPN走了ResNet，没有单独走一遍resnet
        x = self.C1(x)
        x = self.C2(x)
        c2_out = x
        x = self.C3(x)
...
class MaskRCNN(nn.Module):
    ...
    def predict(self, input, mode):
        """
        这个应该就是前向传播，不是使用的默认的forward名称，只要在训练的显式调用就行了
        :param input:
        :param mode:
        :return:
        """
        molded_images = input[0]
        # (1,89) 位数是(1+3+4+num_classes) 分别是(图像id 1个值, 图像形状3个值的list, window数据4个值的list, 81个数据的list，全部为0的81个数据)
        image_metas = input[1]
        ...
        # layer_outputs中每个元素是[得分，概率，偏移],一个6个，
        # 也就是说，zip之后，将得分，概率，偏移分别合并起来了，现在变成了得分，概率，偏移三个列表
        outputs = list(zip(*layer_outputs))
        # 再进行cat，让它的形式变成[1, 特征金字塔所有层锚点*每个点锚框数量,  score/概率=2或者偏移 = 4] 的形状
