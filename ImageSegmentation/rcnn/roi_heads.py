...
1 需要将rpn生成的proposals与groundtruth匹配，匹配后会有一些大于high_threshold阈值的positive proposals和一些小于low_threshold阈值的negative proposals，
也会有一些介于两个阈值的不参与loss计算的ignore proposals。
...
3 将正样本（positive proposals）与proposal region间的deltas(dx, dy, dw, dh)。"""
    ...
    def forward(self,
        ...
        if self.training:
            ...
            matched_idxs = None
        # 使用proposals提供的框在feature map上进行“抠图”，即获取子特征区域，并池化成统一尺度（使得一些列不同尺度的特征图变成同一尺度的特征图）。
        ...
        # 掩模处理了
        if self.has_mask():
            mask_proposals = [p["boxes"] for p in result]  # 应该是mask所在的框
            if self.training:
            ...
            if self.mask_roi_pool is not None:
                # pytorch带的工具 MultiScaleRoIAlign
                # 驾驶获取了框所在的特征
                mask_features = self.mask_roi_pool(features, mask_proposals, image_shapes)
                # type: self.mask_head = mask_rcnn.MaskRCNNHeads
                mask_features = self.mask_head(mask_features)
                # type: self.mask_predictor = mask_rcnn.MaskRCNNPredictor
