...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            # 时候才将输出值转换为预测值。inference的时候则是在Detect中直接转换为预测值，并且
            # inference中计算出来bx,by会加上cx,cy并转换到输入图片的尺寸上，training的时候没有
            ...
            # Obj
            # 此时得到了与target对应的那些anchor的confidence的真实标签，这里计算confidence的真实目
            # 标采用了与yolov1相似的方法，计算真实方框与预测方框的iou作为是否含有目标的优化目标，只不过
            # 这里采用的是giou。其余的anchor的confidence为0
            # 问：用预测值方框与真实方框的giou作为confidence的目标值，这种操作为什么可以学习到是否有目标？？？
            # 预测出来的结果很差的话giou算出出来很小，那最终学习到的confidence值不会很小吗？？？
...
# build_targets()函数
# 在utils/utils.py中，build_targets()函数的目标是找到与target对应的那些anchor在每个yolo层输出tensor中的序号。
# 假设yolo层的输出tensor维度是(b = batch_size数, na = anchor数, w, h, c+5)，则每个anchor的x,y,w,h需要定位到b,na,w,h的坐标，即t
def build_targets(p, targets, model):
    '''
    p:[2x3x20x20x85, 2x3x40x40x85, 2x3x80x80x85]，p是一个list，包含3个yolo层的输出值
    targets: 31x6，表示有31个真实目标，每个目标的标签为(image,class,x,y,w,h),image表示目标所在的图片在这个batch中的序号，class表示目标类别。
    '''
    ...
    for i in range(det.nl):  # 分别对3个yolo层进行处理
        ...
        # by yr,  t: num_targets x 6 = 31x6, every row represents [image, class, x, y, w,
        # h] where x,y,w,h are absolute anchor coordinates and width and height on current scale of current yolo feature map
        # 这里是将每个目标的坐标参数转换成在当前yolo层的特征图的维度上的坐标，因为开始target是中的坐标
        # 是对输入图片归一化的的参数，这里乘以当前特征图的宽、高，得到在当前特征图上的坐标
        ...
        if nt:
            ...
            这里求出了当前yolo层的所有anchor与所有targets的宽、高之间的比值，同时求出了比值的倒数，找
            到每个anchor与targets宽、高的比值，如果一个anchor与一个target的宽、高比值以及target
            与anchor宽、高的比值小于一定阈值的的anchor，则将这个anchor分配到对应的target,即j中对应的元素为true。
            ...
            # 这里每个目标对应的anchor在3个anchor中的序号存放在a中，假设有n个目标，根据前面r,1/r的最
            大值是否小于阈值来决定一个目标是否会被分配anchor，如果3个anchor与这个目标的r,1/r的值都
            满足要求，则这个目标将会被分配3个anchor。所以这里a元素的个数不一定等于目标的数量，而是有
            多少个anchor被分配到这些目标就会有多少个元素。这些被分配的anchor对应的真实标签存放在t中的一行
              重点：anchor和目标之间是相互配对，可能有目标没有分配anchor，也有anchor没有分配目标
            # 问：如果有k个目标处于同一个格子，那这个格子的anchor并没有限定为3个，最多可能有3k个
            anchor分配到这k个目标？？？但是特征图的维度是(b,na,h,w,(c+5))，一个格子最多只有3个
            anchor,这里到底是怎么回事？？？
              问：多个目标处于同一个格子，anchor到底怎么分配的？？？
              回答：根据我在github上对作者进行的提问，发现目前对于多个目标处于同一个格子的情况，他的解
              决办法就是增加anchor数量。
