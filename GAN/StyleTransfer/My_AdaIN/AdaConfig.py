...
oringalModelState = path.join(common_dataset.dataset_dir, r'预训练模型\adain原始权重\model_state.pth')
...
train_style_dir = path.join(common_dataset.dataset_dir, r'动画漫画\动画漫画')
test_content_dir = train_content_dir
test_style_dir = train_style_dir
check_point_dir = 'result'
record = BaseRunRecord.readFromDisk('runRecord.pkl')
if record is None:
    record = RunRecord()
record.createDir()
debugMode = False
