...
def parse_args():
    ...
    path.join(common_dataset.dataset_dir, r'预训练模型\styleganinv_ffhq256_encoder.pth'),
...
def main():
    ...
    assert os.path.exists(Config_IdInvert.invert_list)
    image_list_name = os.path.splitext(os.path.basename(Config_IdInvert.invert_list))[0]
    ...
    image_list = []
    with open(Config_IdInvert.invert_list, 'r') as f:
        for line in f:
    ...
    # Save results.
    os.system(f'cp {Config_IdInvert.invert_list} {output_dir}/image_list.txt')
