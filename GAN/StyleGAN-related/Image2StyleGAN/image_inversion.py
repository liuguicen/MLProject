...
def main():
    ...
    for loop in range(args.loop_time):
        ...
        for i in range(args.w_iteration):
            ...
            mse_loss, perceptual_loss = caluclate_loss(synth_img, img, perceptual_net, img_p, MSE_Loss, 1, 1,
                                                       upsample2d)
            ...
            print("iter{}: loss -- {},  mse_loss --{},  percep_loss --{}".format(loop * loop_iteration + i, loss_np,
                                                                                 loss_m, loss_p))
            if i % 10 == 0:
                save_image(synth_img.clamp(0, 1),
                           "{dir}/{number}.png".format(dir=args.save_dir, number=loop * loop_iteration + i))
        ...
        for i in range(args.n_iteration):
            ...
            synth_img = g_synthesis(dlatent)
            synth_img = (synth_img + 1.0) / 2.0  # Why
            mse_loss, perceptual_loss = caluclate_loss(synth_img, img, perceptual_net, img_p, MSE_Loss, 0, 1,
                                                       upsample2d)
            # adjust ratio to control the gradient part.
            # atio = 0.8
            # loss = (1 - ratio) * mse_loss + ratio * perceptual_loss
            ...
            loss_list.append(loss_np)
            print("iter{}: loss -- {},  mse_loss --{},  percep_loss --{}".format(
                loop * loop_iteration + args.w_iteration + i, loss_np, loss_m, loss_p))
            if i % 10 == 0:
                save_image(synth_img.clamp(0, 1), "{dir}/{number}.png".format(dir=args.save_dir,
