...
2、更新G时，得到G1，如果连续更新G，此时$D^*(x) = \frac{P_data(x)}{P_data(x) + P_G(x)}$是上一次的最优D，V=V(G, D^*)表示的也是上一次的距离，而不是当前$P_{G1}$和$P_{data}$之间的距离度量$JSD(P_{data}|| P_G)$了，所以通常是更新K次D，对应更新一次D，原始论文k取的是1，这里应该也是一种近似，即上次得到的D*接近于这次的分布距离，所以取1:1没大问题。
