...
def create_scores_data():
    ...
    with open(scores_data_path, 'w') as movie_data_file:
...
def create_movies_data():
    # python2可以用file替代open
    with open(movies_data_path, 'w') as movie_data_file:
        writer = csv.writer(movie_data_file)
        # 先写columns_name
...
def add_movie_data(movie_data_list):
    # 写入多行用writerows
    with open(movies_data_path, 'a+') as movie_data_file:
        writer = csv.writer(movie_data_file)
...
def add_score_data(score_data_list):
    # 写入多行用writerows
    with open(scores_data_path, 'a+') as movie_data_file:
        writer = csv.writer(movie_data_file)
...
def read_movie_data():
    movies_data_dict = {}
    with open(movies_data_path_copy, 'r') as movie_data_file:
