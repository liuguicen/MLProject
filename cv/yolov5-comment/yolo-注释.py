...
# 流程图的的模型构建模块主要在yolo.py中完成，根据yaml文件实例化class Model并在Model完成初始化。 parse_model函数完成模型构建，Detect函数对各个yolo层的输入进行处理，得到训练和inference需要的预测结果。

class Model：用于实例化一个模型并完成模型初始化 parse_model函数：根据yaml文件参数创建模型，每个模块保存在一个nn.Sequential中，最终模型的所有模型都 保存到一个nn.Sequential中 Detect函数：对各个yolo层的输入进行处理，得到到最终的预测结果，用于训练和inference。需要注意的是在训 练的时候Detect()并没有将各个yolo层的输入变换成预测值，而是在计算loss的时候才进行转换成预测值
...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            每个anchor预测的方框坐标bx,by,bw,bh计算公式如下，其中cx,cy为anchor所在格子的左上角坐标，stride为当前yolo层的特征维度相比与输入图片的缩小倍数
            ...
            '''
				...
				# 每个yolo层的输出的维度为(bs, anchorNum, na+5)，其中anchorNum是当前yolo层的anchor的总数，最终将所有yolo层的输出在第1个维度上进行concat堆叠，假设所有yolo层的				 # anchor总量为anchorNumTotal，最终的输出是(bs, anchorNumTotal, na+5)
